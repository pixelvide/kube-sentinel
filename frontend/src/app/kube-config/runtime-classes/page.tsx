...
function RuntimeClassesContent() {
    const searchParams = useSearchParams();
    const selectedContext = searchParams.get("context") || "";

    const [resources, setResources] = useState<RuntimeClassInfo[]>([]);
    const [loading, setLoading] = useState(false);
    const searchQuery = searchParams.get("q") || "";
    const [selectedResource, setSelectedResource] = useState<RuntimeClassInfo | null>(null);
                ...
                <Card className="border-none shadow-2xl shadow-black/5 bg-card/50 backdrop-blur-sm overflow-hidden rounded-3xl">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <CardTitle className="text-lg font-bold flex items-center gap-3">
                                    ...
                                    {resources.length} Runtime Classes Found
                                </CardTitle>
                                <CardDescription>
                                    {!selectedContext ? "Select a cluster from the top bar" : "Cluster-wide container runtime configurations"}
                                </CardDescription>
                            </div>
                            <Button
                                variant="outline"
                    ...
                    <CardContent className="p-8">
                        {loading ? (
                            ...
                            <div className="text-center py-12">
                                <Cpu className="h-12 w-12 mx-auto text-muted-foreground/30 mb-4" />
                                <p className="text-muted-foreground text-sm">No Runtime Classes found.</p>
                            </div>
                        ) : (
                                        ...
                                        <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-xs text-muted-foreground">Handler: <span className="text-primary font-mono bg-primary/10 px-1.5 py-0.5 rounded">{res.handler}</span></span>
                                                </div>
                                            </div>
