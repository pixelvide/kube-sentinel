...
function LeasesContent() {
    ...
    const fetchResources = async () => {
        ...
        setResources([]);
        try {
            const res = await fetch(`${API_URL}/kube/leases?context=${selectedContext}&namespace=${selectedNamespace}`, { credentials: "include" });
            if (res.status === 401) {
                    ...
                    <CardHeader className="border-b bg-card/50 px-8 py-6">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    {!selectedContext || !selectedNamespace ? "Select a namespace from the top bar to view Leases" : "Distributed coordination and locking"}
                                </CardDescription>
                            </div>
                            <Button
                                variant="outline"
                                        ...
                                        <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-xs text-muted-foreground">Holder: <span className="text-primary font-mono">{res.holder || "None"}</span></span>
                                                </div>
                                            </div>
