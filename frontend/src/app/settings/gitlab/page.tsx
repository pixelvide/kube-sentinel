...
export default function SettingsPage() {
    ...
    const [agentConfigStatuses, setAgentConfigStatuses] = useState<Record<number, { success: boolean; message: string }>>({});
            ...
            <div className="w-full max-w-5xl space-y-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mt-8">
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="text-muted-foreground text-sm">Configure your GitLab instances and credentials.</p>
                    </div>

                    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                        <DialogTrigger asChild>
                            <Button size="lg" className="h-14 px-8 rounded-2xl font-bold text-sm shadow-xl shadow-primary/20 transition-all active:scale-95 gap-3">
                        ...
                        <DialogContent className="sm:max-w-md border-none shadow-2xl bg-card rounded-[2.5rem] p-0 overflow-hidden">
                            <DialogHeader className="p-8 pb-4 bg-primary/5">
                                ...
                                <DialogDescription className="text-sm">Link a new GitLab host to your private environment.</DialogDescription>
                            </DialogHeader>
                            <div className="p-8 space-y-6">
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="host" className="text-[10px] font-bold uppercase tracking-widest opacity-60 ml-1">GitLab Host</Label>
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="token" className="text-[10px] font-bold uppercase tracking-widest opacity-60 ml-1">Personal Access Token</Label>
                                        <div className="relative">
                                    ...
                                    <div className="flex items-center justify-between p-4 bg-muted/20 rounded-2xl border border-muted/50">
                                        <div className="flex flex-col gap-0.5">
                                            <Label htmlFor="https" className="text-xs font-bold tracking-tight">SSL Security (HTTPS)</Label>
                                            <span className="text-[10px] text-muted-foreground opacity-60 italic">Always recommended</span>
                                        </div>
                                        <Checkbox
                                            id="https"
                                            checked={newIsHttps}
                        ...
                        <div className="p-6 rounded-2xl bg-primary/5 border border-primary/10 space-y-3 sticky top-8">
                            ...
                            <div className="space-y-4 pt-1">
                                <div className="flex items-center justify-between">
                                    <span className="text-[11px] font-medium opacity-60 uppercase tracking-widest">Connected</span>
                                    <span className="text-sm font-bold font-mono">{configs.length}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-[11px] font-medium opacity-60 uppercase tracking-widest">Verified</span>
                                    <span className="text-sm font-bold font-mono text-green-500">{configs.filter(c => c.is_validated).length}</span>
                                </div>
                                <div className="h-px bg-primary/10 w-full" />
                                <p className="text-[10px] leading-relaxed text-muted-foreground italic">
                                    Click "Validate" on any instance to verify connectivity and refresh its persistent status.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        {configs.length === 0 && (
                            <div className="flex flex-col items-center justify-center p-20 border-2 border-dashed rounded-[3rem] border-muted/30 bg-card/10 text-center animate-in fade-in zoom-in duration-500">
                                ...
                                <p className="text-muted-foreground font-medium italic mb-6">No GitLab instances found.</p>
                                <Button variant="outline" onClick={() => setIsModalOpen(true)} className="rounded-xl font-bold uppercase tracking-widest text-[10px] border-2">
                            ...
                            {configs.map((config) => (
                                <div key={config.id} className="space-y-3">
                                    <div className={cn(
                                        ...
                                        editingId === config.id ? "border-primary ring-4 ring-primary/5" : "hover:shadow-xl hover:border-primary/30 hover:-translate-y-1"
                                    )}>
                                        <div className="flex items-center gap-5">
                                            <div className={cn(
                                                "p-3.5 rounded-2xl transition-colors",
                                                config.is_validated ? "bg-green-500/10 text-green-500 shadow-sm shadow-green-500/10" : "bg-muted/50 text-muted-foreground"
                                            )}>
                                                {config.is_validated ? <ShieldCheck className="h-6 w-6" /> : <ShieldAlert className="h-6 w-6 opacity-40" />}
                                            </div>
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-3">
                                                    <span className="font-bold text-base tracking-tight truncate">{config.gitlab_host}</span>
                                                    <div className="flex items-center gap-1.5">
                                                        <span className={cn("text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md border shrink-0", config.is_https ? "bg-green-500/5 border-green-500/20 text-green-600" : "bg-orange-500/5 border-orange-500/20 text-orange-600")}>
                                                ...
                                                {editingId === config.id ? (
                                                    <div className="mt-3 flex flex-col sm:flex-row items-start sm:items-center gap-2">
                                                        ...
                                                        <div className="flex items-center gap-2 w-full sm:w-auto">
                                                            ...
                                                            <Button
                                                                ...
                                                                variant="ghost"
                                                                className="h-9 rounded-lg px-4 font-bold text-[10px] uppercase tracking-wider"
                                                                onClick={() => { setEditingId(null); setEditToken(""); }}
                                                            >
                                                                Cancel
                                                            </Button>
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <p className="text-[11px] font-medium text-muted-foreground/50 tracking-widest font-mono">TOKEN: ••••••••••••••••••</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 mt-6 md:mt-0 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 md:translate-x-2 md:group-hover:translate-x-0">
                                            <Button
                                                ...
                                                className={cn(
                                                    ...
                                                    config.is_validated ? "border-green-500/20 text-green-600 hover:bg-green-500/5" : "border-primary/20 text-primary hover:bg-primary/5"
                                                )}
                                                onClick={() => handleValidate(config.id)}
                                                disabled={isValidatingId === config.id}
                                            >
                                                {isValidatingId === config.id ? "Checking..." : (config.is_validated ? "Re-validate" : "Validate")}
                                            </Button>
                                            ...
                                            <Button
                                                ...
                                                onClick={() => { setEditingId(config.id); setEditToken(""); }}
                                            >
                                    ...
                                    {validationStatuses[config.id] && (
                                        <div className={cn(
                                            ...
                                            validationStatuses[config.id].success ? "bg-green-500/5 border-green-500/20 text-green-700" : "bg-destructive/5 border-destructive/20 text-destructive"
                                        )}>
                                            ...
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-black uppercase tracking-[0.2em]">{validationStatuses[config.id].success ? "Verified Successfully" : "Validation Failed"}</p>
                                                <p className="text-[11px] leading-relaxed opacity-90 break-words font-mono font-medium">{validationStatuses[config.id].message}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="pt-12 space-y-6">
                            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <p className="text-muted-foreground text-sm pl-11">Connect your Kubernetes clusters using GitLab Agents.</p>
                                </div>

                                <Dialog open={isAgentModalOpen} onOpenChange={setIsAgentModalOpen}>
                                    <DialogTrigger asChild>
                                        <Button size="lg" className="h-12 px-6 rounded-xl font-bold text-sm shadow-lg shadow-orange-500/10 transition-all active:scale-95 gap-3 bg-orange-500 hover:bg-orange-600 text-white border-none">
                                    ...
                                    <DialogContent className="sm:max-w-xl border-none shadow-2xl bg-card rounded-[2.5rem] p-0 overflow-hidden">
                                        <DialogHeader className="p-8 pb-4 bg-orange-500/5">
                                            <DialogTitle className="text-xl font-bold tracking-tight text-orange-600">Register GitLab Agent</DialogTitle>
                                            <DialogDescription className="text-sm">Link a new Kubernetes agent from your GitLab projects.</DialogDescription>
                                        </DialogHeader>
                                        <div className="p-8 space-y-6">
                                            <div className="grid grid-cols-1 gap-6">
                                                <div className="space-y-2">
                                                    <Label className="text-[10px] font-bold uppercase tracking-widest opacity-60 ml-1">GitLab Instance</Label>
                                                    <Select value={selectedGitlabId} onValueChange={setSelectedGitlabId}>
                                                        <SelectTrigger className="h-12 border-muted bg-muted/30 rounded-xl focus:ring-orange-500/20">
                                                            <SelectValue placeholder="Select instance" />
                                                        </SelectTrigger>
                                                        <SelectContent className="rounded-xl border-none shadow-2xl">
                                                            {configs.filter(c => c.is_validated).map((config) => (
                                                                <SelectItem key={config.id} value={config.id.toString()} className="rounded-lg">
                                                                    {config.gitlab_host}
                                                                </SelectItem>
                                                            ))}
                                                            {configs.filter(c => c.is_validated).length === 0 && (
                                                                <div className="p-4 text-[11px] text-muted-foreground italic">No verified instances available.</div>
                                                            )}
                                                        </SelectContent>
                                                    </Select>
                                                </div>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    <div className="space-y-2">
                                                        <Label htmlFor="agent_id" className="text-[10px] font-bold uppercase tracking-widest opacity-60 ml-1">Agent ID</Label>
                                                        <Input
                                                            id="agent_id"
                                                    ...
                                                    <div className="space-y-2">
                                                        <Label htmlFor="agent_repo" className="text-[10px] font-bold uppercase tracking-widest opacity-60 ml-1">Agent Repository</Label>
                                                        <Input
                                                            id="agent_repo"
                                ...
                                {agentConfigs.map((agent) => (
                                    <div key={agent.id} className="space-y-4">
                                        <div className="group flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-card/40 backdrop-blur-md border border-muted/20 rounded-[2rem] hover:border-primary/20 transition-all duration-300">
                                            <div className="flex items-center gap-5">
                                                <div className={cn(
                                                    ...
                                                    agent.is_configured ? "bg-green-500/10 text-green-500 shadow-sm shadow-green-500/5" : "bg-primary/5 text-primary"
                                                )}>
                                                    <Server className="h-5 w-5" />
                                                </div>
                                                <div className="space-y-1">
                                                    <div className="flex items-center gap-2 flex-wrap">
                                                        <span className="text-xs font-bold opacity-60 uppercase tracking-widest">{agent.gitlab_config.gitlab_host}</span>
                                                        <ChevronRight className="h-3 w-3 opacity-20" />
                                                        <span className="font-bold text-sm tracking-tight">Agent Id: {agent.agent_id}</span>
                                                        {agent.is_configured && (
                                                    ...
                                                    <p className="text-[11px] font-medium text-muted-foreground/70 font-mono tracking-tight break-all">{agent.agent_repo}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2 md:opacity-0 md:group-hover:opacity-100 transition-all">
                                                <Button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        agent.is_configured ? "border-green-500/20 text-green-600 hover:bg-green-500/5" : "border-primary/20 text-primary hover:bg-primary/5"
                                                    )}
                                                    onClick={() => handleConfigureAgent(agent.id)}
                                                    disabled={configuringId === agent.id}
                                                >
                                                    {configuringId === agent.id ? "Configuring..." : (agent.is_configured ? "Re-configure" : "Configure")}
                                                </Button>
                                                <div className="w-px h-6 bg-muted/60 mx-1" />
                                                <Button
                                                    variant="ghost"
                                        ...
                                        {agentConfigStatuses[agent.id] && (
                                            <div className={cn(
                                                "p-4 rounded-2xl border flex items-start gap-4 animate-in fade-in slide-in-from-top-4 duration-500 mx-4",
                                                agentConfigStatuses[agent.id].success ? "bg-green-500/5 border-green-500/20 text-green-700" : "bg-destructive/5 border-destructive/20 text-destructive"
                                            )}>
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-[10px] font-black uppercase tracking-[0.2em]">{agentConfigStatuses[agent.id].success ? "Configuration Successful" : "Configuration Failed"}</p>
                                                    <p className="text-[11px] leading-relaxed opacity-90 break-words font-mono font-medium">{agentConfigStatuses[agent.id].message}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
