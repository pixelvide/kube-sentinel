...
export function Sidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
    ...
    useEffect(() => {
        ...
        const activeItem = NAVIGATION_CONFIG.find(
                ...
                <div className="p-8 pb-4 flex flex-col gap-4">
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <Button
                            variant="ghost"
                            size="icon"
                            className="lg:hidden text-sidebar-foreground/50"
                            onClick={onClose}
                        >
                            <X className="h-5 w-5" />
                        </Button>
                ...
                <nav className="flex-1 px-3 space-y-1.5 overflow-y-auto">
                    {(() => {
                        ...
                        const categories = Array.from(
                            new Set(NAVIGATION_CONFIG.map((item) => item.category).filter(Boolean))
                        ) as string[];
                        ...
                        return order.map((orderItem) => {
                            if (orderItem.type === "path") {
                                    ...
                                    >
                                        ...
                                        >
                                            <Icon
                                                className={cn(
                                                    "h-4 w-4",
                                                    isActive(item.path) ? "text-primary" : "opacity-60"
                                                )}
                                            />
                                ...
                                const items = NAVIGATION_CONFIG.filter(
                                    (i) => i.category === categoryName && !i.hideFromSidebar
                                );
                                    ...
                                    <div key={categoryName} className="space-y-1">
                                        ...
                                        {isOpen && (
                                            <div className="ml-4 space-y-1">
                                                ...
                                                {/* Dynamic CRD Groups */}
                                                {categoryName === "Custom Resources" &&
                                                    (Object.entries(groupedCrds) as [string, any[]][])
                                                        .sort()
                                                        .map(([group, groupCrds]) => (
