...
export function GitlabConfigManagement() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(
        t('settings.gitlab.deleteError', 'Failed to delete configuration')
      )
    }
  }
      ...
      <div className="flex justify-between items-center">
        <div className="space-y-1">
          <h3 className="text-lg font-medium">
            {t('settings.gitlab.title', 'GitLab Configuration')}
          </h3>
          <p className="text-sm text-muted-foreground">
            {t(
              'settings.gitlab.subtitle',
              'Manage your GitLab tokens for various hosts.'
            )}
          </p>
        </div>
        ...
        >
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editingConfig
                  ? t('settings.gitlab.edit', 'Edit Configuration')
                  : t('settings.gitlab.add', 'Add Configuration')}
              </DialogTitle>
              <DialogDescription>
                {t(
                  'settings.gitlab.description',
              ...
              >
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t('settings.gitlab.host', 'GitLab Host')}
                      </FormLabel>
                      <Select
